...
layuiXtree.prototype.Loading = function (options) {
    ...
    _this.xloading.setAttribute('class',
    ...
    _this.xloading.style.marginLeft = _this._container.offsetWidth / 2 - 25
        + 'px';
    _this.xloading.style.marginTop = _this._container.offsetHeight / 2 - 50
        + 'px';
...
layuiXtree.prototype.CreateCkAll = function () {
    ...
    if (_this._ckall) {
        ...
        _this._domStr += '<input type="checkbox" class="layui-xtree-checkbox layui-xtree-ckall" title="全选" lay-skin="primary" lay-filter="xtreeckall'
            + _this._containerid + '">';
        _this._domStr += '</div>';
    }
}
    ...
    if (d.length > 0) {
        for (i in d) {
            ...
            if (d[i].data.length > 0) {
                _this._domStr += '<i class="layui-icon layui-xtree-icon" data-xtree="'
                    + (_this._isopen ? '1' : '0') + '">' + (_this._isopen
                        ? _this._iconOpen : _this._iconClose) + '</i>';
            } else {
                _this._domStr += '<i class="layui-icon layui-xtree-icon-null">'
                    + _this._iconEnd + '</i>';
                xtree_isend = 'data-xend="1"';
                xtree_ischecked = d[i].checked ? ' checked ' : '';
                xtree_isdisabled = d[i].disabled ? ' disabled="disabled" ' : '';
            }
            _this._domStr += '<input type="checkbox" class="layui-xtree-checkbox" '
                + xtree_isend + xtree_ischecked + xtree_isdisabled + ' value="'
                + d[i].value + '" title="' + d[i].title
                + '" lay-skin="primary" lay-filter="xtreeck'
                + _this._containerid + '">';
...
layuiXtree.prototype.Rendering = function () {
    ...
    for (var i = 0; i < xtree_ckitems.length; i++) {
        if (xtree_ckitems[i].getAttribute('data-xend') == '1'
            && xtree_ckitems[i].checked) {
    ...
    for (var i = 0; i < xtree_icons.length; i++) {
        ...
        xtree_icons[i].style.color = _this._isopen ? _this._iconOpenColor
            : _this._iconCloseColor;
        ...
        xtree_icons[i].onclick = function () {
            ...
            if (this.getAttribute('data-xtree') == 1) {
                for (var j = 0; j < xtree_chi.length; j++) {
                    if (xtree_chi[j].getAttribute('class')
                        == 'layui-xtree-item') {
    ...
    _this._form.on('checkbox(xtreeck' + _this._containerid + ')',
        function (da) {
            //获取当前点击复选框的容器下面的所有子级容器 
            var xtree_chis = da.elem.parentNode.getElementsByClassName(
                'layui-xtree-item');
            //遍历它们，选中状态与它们的父级一致（类似全选功能）
            for (var i = 0; i < xtree_chis.length; i++) {
                if (!_this.getChildByClassName(xtree_chis[i],
                        'layui-xtree-checkbox')[0].disabled) {
                    _this.getChildByClassName(xtree_chis[i],
                        'layui-xtree-checkbox')[0].checked = da.elem.checked;
                    if (da.elem.checked) {
                        _this.getChildByClassName(
                            xtree_chis[i],
                            'layui-xtree-checkbox')[0].nextSibling.classList.add(
                            'layui-form-checked');
                    } else {
                        _this.getChildByClassName(xtree_chis[i],
                            'layui-xtree-checkbox')[0].nextSibling.classList.remove(
                            'layui-form-checked');
                    }
                }
            }
    ...
    for (var i = 0; i < _xtree_disableds.length; i++) {
        _xtree_disableds[i].getElementsByTagName(
            'span')[0].style.color = "#B5B5B5";
    }

    //全选按钮
    if (_this._ckall) {
        _this._form.on('checkbox(xtreeckall' + _this._containerid + ')',
            function (data) {
                var xtree_allck = data.elem.parentNode.parentNode.getElementsByClassName(
                    'layui-form-checkbox');
                for (var i = 0; i < xtree_allck.length; i++) {
                    if (xtree_allck[i].getAttribute('class').indexOf(
                            'layui-checkbox-disbaled') == -1) {
                        if (data.elem.checked) {
                            xtree_allck[i].classList.add('layui-form-checked');
                        }
                        else {
                            xtree_allck[i].classList.remove(
                                'layui-form-checked');
                        }
                        xtree_allck[i].parentNode.getElementsByClassName(
                            'layui-xtree-checkbox')[0].checked = data.elem.checked;
                    }
                }
                _this._ckallSuccess();
            });
...
layuiXtree.prototype.ParendCheck = function (ckelem) {
    ...
    if (xtree_p.getAttribute('class') == 'layui-xtree-item') {
        ...
        for (var i = 0; i < xtree_all.length; i++) {
            if (_this.getChildByClassName(xtree_all[i],
                    'layui-xtree-checkbox')[0].checked) {
                xtree_count++;
            }
        }

        if (xtree_count <= 0) {
            _this.getChildByClassName(xtree_p,
                'layui-xtree-checkbox')[0].checked = false;
            _this.getChildByClassName(xtree_p,
                'layui-xtree-checkbox')[0].nextSibling.classList.remove(
                'layui-form-checked');
        } else {
            _this.getChildByClassName(xtree_p,
                'layui-xtree-checkbox')[0].checked = true;
            _this.getChildByClassName(xtree_p,
                'layui-xtree-checkbox')[0].nextSibling.classList.add(
                'layui-form-checked');
        }
        this.ParendCheck(
            _this.getChildByClassName(xtree_p, 'layui-xtree-checkbox')[0]);
    }
}
...
layuiXtree.prototype.ParentCheckboxChecked = function (e) {
    ...
    if (e.parentNode.parentNode.getAttribute('class') == 'layui-xtree-item') {
        var _pe = _this.getChildByClassName(e.parentNode.parentNode,
            'layui-xtree-checkbox')[0];
        _pe.checked = true;
...
//获取全部选中的末级checkbox对象
layuiXtree.prototype.GetEndChecked = function () {
    var _this = this;
    var arr = new Array();
    var arrIndex = 0;
...
layuiXtree.prototype.GetParent = function (a) {
    ...
    for (var i = 0; i < cks.length; i++) {
        if (cks[i].value == a) {
            if (cks[i].parentNode.parentNode.getAttribute('id')
                == _this._container.getAttribute('id')) {
                return null;
            }
            return _this.getChildByClassName(cks[i].parentNode.parentNode,
                'layui-xtree-checkbox')[0];
        }
    }
    return null;
