...
var CodeMirror = (function () {
    ...
    function CodeMirror(place, givenOptions) {
        ...
        wrapper.className = "CodeMirror" + (options.lineWrapping
            ? " CodeMirror-wrap" : "");
        ...
        var inputDiv = wrapper.firstChild, input = inputDiv.firstChild,
            ...
            mover = code.firstChild, gutter = mover.firstChild,
        ...
        if (/AppleWebKit/.test(navigator.userAgent) && /Mobile\/\w+/.test(
                navigator.userAgent)) {
        ...
        var mode, doc = new BranchChunk([new LeafChunk([new Line("")])]), work,
            focused;
        loadMode();
        ...
        var shiftSelecting, lastClick, lastDoubleClick, draggingText,
            overwrite = false;
        ...
        connect(scroller, "scroll", function () {
            ...
            if (options.fixedGutter) {
                gutter.style.left = scroller.scrollLeft
                    + "px";
            }
            if (options.onScroll) {
        ...
        var instance = wrapper.CodeMirror = {
            ...
            setOption: function (option, value) {
                ...
                if (option == "lineNumbers" || option == "gutter" || option
                    == "firstLineNumber" || option == "theme") {
            ...
            indentLine: operation(function (n, dir) {
                if (isLine(n)) {
                    indentLine(n,
                        dir == null ? "smart" : dir ? "add" : "subtract");
                }
            }),
            ...
            addWidget: function (pos, node, scroll, vert, horiz) {
                ...
                if (vert == "over") {
                    ...
                    var vspace = Math.max(scroller.offsetHeight, doc.height
                        * textHeight()),
                        hspace = Math.max(code.clientWidth,
                            lineSpace.clientWidth) - paddingLeft();
                    if (pos.yBot + node.offsetHeight > vspace && pos.y
                        > node.offsetHeight) {
            ...
            setCursor: operation(function (line, ch, user) {
                if (ch == null && typeof line.line == "number") {
                    setCursor(
                        line.line, line.ch, user);
                } else {
                    setCursor(line, ch, user);
                }
            }),
            setSelection: operation(function (from, to, user) {
                (user ? setSelectionUser : setSelection)(clipPos(from),
                    clipPos(to || from));
            }),
            ...
            setLine: operation(function (line, text) {
                if (isLine(line)) {
                    replaceRange(text, {line: line, ch: 0},
                        {line: line, ch: getLine(line).text.length});
                }
            }),
            removeLine: operation(function (line) {
                if (isLine(line)) {
                    replaceRange("", {line: line, ch: 0},
        ...
        function onMouseDown(e) {
                ...
                if (n.parentNode == gutterText) {
                    if (options.onGutterClick) {
                        options.onGutterClick(instance, indexOf(
                            gutterText.childNodes, n) + showingFrom, e);
                    }
            ...
            } else if (lastClick && lastClick.time > now - 400 && posEq(
                    lastClick.pos, start)) {
            ...
            if (dragAndDrop && !posEq(sel.from, sel.to) &&
                ...
                var up = connect(targetDocument, "mouseup",
            ...
            var move = connect(targetDocument, "mousemove",
        ...
        function handleKeyBinding(e) {
            var name = keyNames[e.keyCode], next = keyMap[options.keyMap].auto,
                bound, dropShift;
            ...
            if (e.shiftKey && (bound = lookupKey("Shift-" + name,
                    options.extraKeys, options.keyMap))) {
                dropShift = true;
            } else {
                bound = lookupKey(name, options.extraKeys, options.keyMap);
            }
            if (typeof bound == "string") {
                if (commands.propertyIsEnumerable(
                        bound)) {
        ...
        function onKeyDown(e) {
            ...
            if (options.onKeyEvent && options.onKeyEvent(instance,
                    addStop(e))) {
                return;
            }
            ...
            if (window.opera) {
                ...
                if (!handled && (mac ? e.metaKey : e.ctrlKey) && e.keyCode
                    == 88) {
                    replaceSelection("");
                }
            }
        ...
        function updateLinesNoUndo(from, to, newText, selFrom, selTo) {
            ...
            var nlines = to.line - from.line, firstLine = getLine(from.line),
        ...
        function replaceRange(code, from, to) {
            ...
            function adjustPos(pos) {
                ...
                var ch = pos.ch;
                if (pos.line == to.line) {
                    ch += code[code.length - 1].length - (to.ch - (to.line
                    == from.line ? from.ch : 0));
                }
        ...
        function replaceRange1(code, from, to, computeSel) {
            ...
            var newSel = computeSel(
                {line: from.line + code.length - 1, ch: endch});
        ...
        function readInput() {
            if (leaveInputAlone || !focused || hasSelection(
                    input)) {
                return false;
            }
            ...
            if (same < prevInput.length) {
                ...
                sel.to = {
                    line: sel.to.line,
                    ch: Math.min(getLine(sel.to.line).text.length, sel.to.ch
                        + (text.length - same))
                };
            }
        ...
        function scrollEditorIntoView() {
            ...
            var winH = window.innerHeight || Math.max(
        ...
        function scrollIntoView(x1, y1, x2, y2) {
            ...
            x1 += pl;
            x2 += pl;
            var screen = scroller.clientHeight, screentop = scroller.scrollTop,
                scrolled = false, result = true;
            if (y1 < screentop) {
        ...
        function updateDisplay(changes, suppressCallback) {
            ...
            var intact = changes === true ? [] : computeIntact(
                [{from: showingFrom, to: showingTo, domStart: 0}], changes);
            ...
            var different = from != showingFrom || to != showingTo || lastSizeC
                != scroller.clientHeight + th;
        ...
        function computeIntact(intact, changes) {
            for (var i = 0, l = changes.length || 0; i < l; ++i) {
                ...
                for (var j = 0, l2 = intact.length; j < l2; ++j) {
                    ...
                    if (change.to <= range.from && change.diff) {
                    ...
                    } else if (change.to <= range.from || change.from
                        >= range.to) {
                        intact2.push(range);
                    } else {
                        ...
                        if (change.to < range.to) {
                            intact2.push({
                                ...
                                domStart: range.domStart + (change.to
                                    - range.from)
                            });
                        }
                    }
        ...
        function patchDisplay(from, to, intact) {
            ...
            var nextIntact = intact.shift(), curNode = lineDiv.firstChild,
                j = from;
            var sfrom = sel.from.line, sto = sel.to.line,
        ...
        function updateGutter() {
            ...
            var minwidth = String(doc.size).length,
                firstNode = gutterText.firstChild, val = eltText(firstNode),
                pad = "";
        ...
        function updateCursor() {
            ...
            if (posEq(sel.from, sel.to)) {
                ...
                cursor.style.left = (options.lineWrapping ? Math.min(pos.x,
                    lineSpace.offsetWidth) : pos.x) + "px";
        ...
        function setShift(val) {
            if (val) {
                shiftSelecting = shiftSelecting || (sel.inverted ? sel.to
                    : sel.from);
            } else {
                shiftSelecting = null;
            }
        ...
        function findPosH(dir, unit) {
            var end = sel.inverted ? sel.from : sel.to, line = end.line,
                ch = end.ch;
            var lineObj = getLine(line);

            function findNextLine() {
                for (var l = line + dir, e = dir < 0 ? -1 : doc.size; l != e;
                    l += dir) {
                    var lo = getLine(l);
                    if (!lo.hidden) {
                        line = l;
        ...
        function selectWordAt(pos) {
            ...
            setSelectionUser({line: pos.line, ch: start},
        ...
        function indentLine(n, how) {
            ...
            if (!diff) {
                ...
                if (options.indentWithTabs) {
                    for (var i = Math.floor(indentation / options.tabSize); i;
                        --i) {
                        pos += options.tabSize;
                        indentString += "\t";
                    }
            ...
            replaceRange(indentString, {line: n, ch: 0},
        ...
        function computeTabText() {
            for (var str = '<span class="cm-tab">', i = 0; i < options.tabSize;
                ++i) {
                str += " ";
            }
            return str + "</span>";
        ...
        function charFromX(line, x) {
            ...
            function getX(len) {
                measure.innerHTML = "<pre><span>" + lineObj.getHTML(null, null,
                    false, tabText, len) + "</span></pre>";
            ...
            for (; ;) {
                ...
                if (estX <= x && estimated < to) {
                    estimated = Math.min(to,
                        Math.ceil(estimated * 1.2));
                } else {
                    toX = estX;
                    to = estimated;
                    break;
        ...
        function measureLine(line, ch) {
            ...
            if (options.lineWrapping) {
                ...
                extra = htmlEscape(line.text.slice(ch + 1,
                    end < 0 ? line.text.length : end + (ie ? 5 : 0)));
            }
            measure.innerHTML = "<pre>" + line.getHTML(null, null, false,
                tabText, ch) +
                '<span id="CodeMirror-temp-' + tempId + '">'
                + htmlEscape(line.text.charAt(ch) || " ") + "</span>" +
                extra + "</pre>";
        ...
        function localCoords(pos, inLineWrap) {
            var x, lh = textHeight(),
                y = lh * (heightAtLine(doc, pos.line) - (inLineWrap
                    ? displayOffset : 0));
            if (pos.ch == 0) {
                x = 0;
            } else {
        ...
        function coordsChar(x, y) {
            ...
            var th = textHeight(), cw = charWidth(),
            ...
            var from = 0, fromX = 0, to = text.length, toX;
            // Guess a suitable upper bound for our search.
            var estimated = Math.min(to,
                Math.ceil((x + innerOff * scroller.clientWidth * .9) / cw));
            for (; ;) {
                var estX = getX(estimated);
                if (estX <= x && estimated < to) {
                    estimated = Math.min(to,
        ...
        function posFromMouse(e, liberal) {
            ...
            // (and !liberal).
            if (!liberal && (x - offW.left > scroller.clientWidth || y
                    - offW.top > scroller.clientHeight)) {
                return null;
            }
        ...
        function onContextMenu(e) {
            ...
            if (posEq(sel.from, sel.to) || posLess(pos, sel.from) || !posLess(
                    pos, sel.to)) {
        ...
        function matchBrackets(autoclear) {
            var head = sel.inverted ? sel.from : sel.to,
                line = getLine(head.line), pos = head.ch - 1;
            var match = (pos >= 0 && matching[line.text.charAt(pos)])
                || matching[line.text.charAt(++pos)];
            if (!match) {
                return;
            }
            var ch = match.charAt(0), forward = match.charAt(1) == ">",
                d = forward ? 1 : -1, st = line.styles;
            ...
            function scan(line, from, to) {
                ...
                var st = line.styles, pos = forward ? 0 : line.text.length - 1,
                    cur;
                for (var i = forward ? 0 : st.length - 2,
                    e = forward ? st.length : -2; i != e; i += 2 * d) {
                    var text = st[i];
                    ...
                    for (var j = forward ? 0 : text.length - 1,
                        te = forward ? text.length : -1; j != te;
                        j += d, pos += d) {
                        if (pos >= from && pos < to && re.test(
                                cur = text.charAt(j))) {
                            var match = matching[cur];
                            if (match.charAt(1) == ">" == forward) {
                                stack.push(
                                    cur);
                            } else if (stack.pop() != match.charAt(
                                    0)) {
            ...
            for (var i = head.line,
                e = forward ? Math.min(i + 100, doc.size) : Math.max(-1, i
                    - 100); i != e; i += d) {
                var line = getLine(i), first = i == head.line;
                var found = scan(line, first && forward ? pos + 1 : 0,
            ...
            var one = markText({line: head.line, ch: pos},
                {line: head.line, ch: pos + 1}, style),
                two = found.pos != null && markText({line: i, ch: found.pos},
                    {line: i, ch: found.pos + 1}, style);
                ...
                doc.iter(i, doc.size, function (line) {
                    ...
                    if (compare) {
                        ...
                        if (changed !== false || !hadState) {
                            unchanged = 0;
                        } else if (++unchanged > 3 && (!mode.indent
                                || mode.indent(hadState, "") == mode.indent(
                                    state, ""))) {
                            return true;
                        }
                    }
        ...
        function endOperation() {
            ...
            if (focused && !leaveInputAlone &&
                (updateInput === true || (updateInput !== false
                    && selectionChanged))) {
    ...
    var commands = CodeMirror.commands = {
        ...
        goLineStartSmart: function (cm) {
            ...
            cm.setCursor(cur.line,
                cur.ch <= firstNonWS && cur.ch ? 0 : firstNonWS, true);
        },
    ...
    function lookupKey(name, extraMap, map) {
        ...
        return extraMap ? lookup(name, extraMap, map) : lookup(name,
            keyMap[map]);
    }
    ...
    StringStream.prototype = {
        ...
        match: function (pattern, consume, caseInsensitive) {
            if (typeof pattern == "string") {
                ...
                if (cased(this.string).indexOf(cased(pattern), this.pos)
                    == this.pos) {
                    if (consume !== false) {
    ...
    MarkedText.prototype = {
        ...
        clipTo: function (fromOpen, from, toOpen, to, diff) {
            ...
            if (fromOpen && to > this.from && (to < this.to || this.to
                    == null)) {
                this.from = null;
            }
            if (toOpen && (from < this.to || this.to == null) && (from
    ...
    Line.prototype = {
        ...
        highlight: function (mode, state, tabSize) {
            ...
            while (!stream.eol()) {
                ...
                if (pos && st[pos - 1] == style) {
                    ...
                    if (!changed && (st[pos + 1] != style || (pos && st[pos - 2]
                            != prevWord))) {
                        changed = true;
                    }
                    st[pos++] = substr;
            ...
            return changed || (st.length < 5 && this.text.length < 10 ? null
                : false);
        },
        ...
        getHTML: function (sfrom, sto, includePre, tabText, endAt) {
            ...
            if (includePre) {
                html.push(
                    this.className ? '<pre class="' + this.className + '">'
                        : "<pre>");
            }
            ...
            function span(text, style) {
                ...
                first = false;
                if (style) {
                    html.push('<span class="', style, '">',
                        htmlEscape(text).replace(/\t/g, tabText), "</span>");
                } else {
            ...
            if (!allText && endAt == null) {
                ...
                while (pos < len) {
                    ...
                    while (mark && mark.to != null && mark.to
                    <= pos) {
                        nextMark();
                    }
                    if (mark) {
        ...
        cleanUp: function () {
            this.parent = null;
            if (this.marked) {
                for (var i = 0, e = this.marked.length; i < e;
                    ++i) {
    ...
    LeafChunk.prototype = {
        ...
        remove: function (at, n, callbacks) {
            for (var i = at, e = at + n; i < e; ++i) {
                ...
                line.cleanUp();
                if (line.handlers) {
                    for (var j = 0; j < line.handlers.length;
                        ++j) {
        ...
        insertHeight: function (at, lines, height) {
            ...
            for (var i = 0, e = lines.length; i < e;
                ++i) {
                lines[i].parent = this;
            }
        },
    ...
    BranchChunk.prototype = {
        ...
        collapse: function (lines) {
            for (var i = 0, e = this.children.length; i < e;
                ++i) {
                ...
                if (at <= sz) {
                    ...
                    if (child.lines && child.lines.length > 50) {
                        while (child.lines.length > 50) {
                            var spilled = child.lines.splice(child.lines.length
                                - 25, 25);
    ...
    History.prototype = {
        addChange: function (start, added, old) {
            ...
            if (time - this.time > 400 || !last ||
                last.start > start + added || last.start + last.added < start
                - last.added + last.old.length) {
...
CodeMirror.defineMode("xml", function (config, parserConfig) {
    ...
    return {
        ...
        compareStates: function (a, b) {
            if (a.indented != b.indented || a.tokenize
                != b.tokenize) {
                return false;
            }
            for (var ca = a.context, cb = b.context; ;
                ca = ca.prev, cb = cb.prev) {
                if (!ca || !cb) {
                    return ca == cb;
                }
...
CodeMirror.defineMode("javascript", function (config, parserConfig) {
    ...
    function jsTokenBase(stream, state) {
        ...
        if (ch == '"' || ch == "'") {
            ...
            return (known && state.kwAllowed) ? ret(known.type, known.style,
                word) : ret("variable", "variable", word);
        }
    }
    ...
    function pushlex(type, info) {
        var result = function () {
            var state = cx.state;
            state.lexical = new JSLexical(state.indented, cx.stream.column(),
    ...
    function statement(type) {
        if (type == "var") {
            return cont(pushlex("vardef"), vardef1, expect(";"),
                poplex);
        }
        if (type == "keyword a") {
        ...
        if (type == "for") {
            return cont(pushlex("form"), expect("("),
        ...
        if (type == "catch") {
            return cont(pushlex("form"), pushcontext,
    ...
    function expression(type) {
        ...
        if (type == "(") {
            return cont(pushlex(")"), expression, expect(")"),
    ...
    function functiondef(type, value) {
        ...
        if (type == "(") {
            return cont(pushlex(")"), pushcontext,
                commasep(funarg, ")"), poplex, statement, popcontext);
        }
    }
    ...
    return {
        ...
        token: function (stream, state) {
            ...
            state.reAllowed = type == "operator" || type == "keyword c"
                || type.match(/^[\[{}\(,;:]$/);
        ...
        copyState: function (state) {
            if (state.localState) {
                var local = CodeMirror.copyState(
                    state.token == css ? cssMode : jsMode, state.localState);
            }
            return {
