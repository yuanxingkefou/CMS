...
(function (H) {
    ...
    Chart.prototype.showDrillUpButton = function () {
        ...
        if (!this.drillUpButton) {
            ...
            .add()
            .align(buttonOptions.position, false, buttonOptions.relativeTo
                || 'plotBox');
        } else {
    ...
    Chart.prototype.drillUp = function () {
        ...
        if (newSeries.xAxis) {
            newSeries.xAxis.setExtremes(oldExtremes.xMin, oldExtremes.xMax,
                false);
            newSeries.yAxis.setExtremes(oldExtremes.yMin, oldExtremes.yMax,
                false);
        }
    ...
    PieSeries.prototype.animateDrilldown = function (init) {
        var level = this.chart.drilldownLevels[this.chart.drilldownLevels.length
            - 1],
    ...
    ColumnSeries.prototype.animateDrilldown = function (init) {
        var animateFrom = this.chart.drilldownLevels[this.chart.drilldownLevels.length
            - 1].shapeArgs,
            animationOptions = this.chart.options.drilldown.animation;
                ...
                each(this.points, function (point) {
                    ...
                    graphic.animate(level.shapeArgs, H.merge(animationOptions, {
                        ...
                        step: function (val, fx) {
                            if (fx.prop === 'start') {
                                this.attr({
                                    fill: tweenColors(startColor,
                                        H.Color(level.color).rgba, fx.pos)
                                });
                            }
                        },
                        complete: function () {
        ...
        if (point.drilldown) {
            ...
            if (tickLabel) {
                if (!tickLabel._basicStyle) {
                    tickLabel._basicStyle = tickLabel.element.getAttribute(
                        'style');
                }
                tickLabel
    ...
    for (type in seriesTypes) {
        if (seriesTypes[type].prototype.supportsDrilldown) {
            wrap(seriesTypes[type].prototype, 'drawTracker',
                drawTrackerWrapper);
        }
    }
