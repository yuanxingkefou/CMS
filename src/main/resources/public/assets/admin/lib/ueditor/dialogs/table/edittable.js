...
(function () {
    ...
    editTable.prototype = {
        init: function () {
            ...
            sorttable.checked = !!(enablesortState < 0 && disablesortState
                >= 0);
            sorttable.disabled = !!(enablesortState < 0 && disablesortState
                < 0);
            sorttable.title = enablesortState < 0 && disablesortState < 0
                ? lang.errorMsg : '';
        ...
        captionHanler: function () {
            ...
            if (caption.checked) {
                ...
                domUtils.remove(
                    domUtils.getElementsByTagName(example, 'caption')[0]);
            }
        },
        ...
        getColor: function () {
            var start = editor.selection.getStart(), color,
                cell = domUtils.findParentByTagName(start,
                    ["td", "th", "caption"], true);
    ...
    dialog.onok = function () {
        ...
        for (var i in checks) {
            var cmds = checks[i].split(" "),
                input = $G("J_" + i);
            if (input["checked"]) {
                editor.queryCommandState(cmds[0]) != -1 && editor.execCommand(
                    cmds[0]);
            } else {
                editor.queryCommandState(cmds[1]) != -1 && editor.execCommand(
                    cmds[1]);
            }
        }
