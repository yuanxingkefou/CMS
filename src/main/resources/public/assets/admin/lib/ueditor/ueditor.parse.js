...
(function () {
    ...
    (function () {
        ...
        var utils = {
            ...
            extend: function (t, s) {
                var a = arguments,
                    notCover = this.isBoolean(a[a.length - 1]) ? a[a.length - 1]
                        : false,
                    len = this.isBoolean(a[a.length - 1]) ? a.length - 1
                        : a.length;
                for (var i = 1; i < len; i++) {
                    var x = a[i];
                    for (var k in x) {
            ...
            hasClass: function (element, className) {
                className = className.replace(/(^[ ]+)|([ ]+$)/g, '').replace(
                    /[ ]{2,}/g, ' ').split(' ');
            ...
            removeClass: function (elm, classNames) {
                classNames = this.isArray(classNames) ? classNames : this.trim(
                    ...
                    while (k--) {
                        ...
                        if (element.removeEventListener) {
                            ...
                            if (handler._d && handler._d[key]) {
                                var index = utils.indexOf(handler._d.els,
                                    element);
                                if (index != -1) {
            ...
            loadFile: function () {
                ...
                return function (doc, obj, fn) {
                    ...
                    element.onload = element.onreadystatechange = function () {
                        if (!this.readyState || /loaded|complete/.test(
                                this.readyState)) {
        ...
        uParse = function (selector, opt) {
            utils.domReady(function () {
                ...
                utils.each(contents, function (v) {
                    UE.parse.load(
                        utils.extend({root: v, selector: selector}, opt))
                })
            })
        }
    })();
        ...
        if (pres.length) {
            if (typeof XRegExp == "undefined") {
                ...
                if (this.rootPath !== undefined) {
                    jsurl = utils.removeLastbs(this.rootPath)
                        + '/third-party/SyntaxHighlighter/shCore.js';
                    cssurl = utils.removeLastbs(this.rootPath)
    ...
    UE.parse.register('table', function (utils) {
        ...
        if (tables.length) {
            ...
            utils.cssRule('table',
                ...
                +
                selector + ' table.sortEnabled tr.firstRow th,' + selector
                ...
                + this.rootPath + 'themes/default/images/sortable.png);}' +
                selector + ' table.sortEnabled tr.firstRow th:hover,' + selector
            ...
            utils.each(tables, function (table) {
                if (/\bsortEnabled\b/.test(table.className)) {
                    utils.on(table, 'click', function (e) {
                        ...
                        if (colIndex != -1) {
                            sortTable(table, colIndex, me.tableSortCompareFn
                                || sortType);
                            updateTable(table);
                        }
                    });
            ...
            function sortTable(table, sortByCellIndex, compareFn) {
                ...
                var Fn = {
                    ...
                    'orderbynum': function (td1, td2) {
                        var value1 = td1[utils.isIE ? 'innerText'
                            : 'textContent'].match(/\d+/),
                            value2 = td2[utils.isIE ? 'innerText'
                                : 'textContent'].match(/\d+/);
                        if (value1) {
                            value1 = +value1[0];
                        }
                ...
                table.setAttribute('data-sort-type',
                ...
                if (!lastRowIndex) {
                    ...
                    tbody.insertBefore(fragment,
                        rows[lastRowIndex - range.endRowIndex
    ...
    UE.parse.register('charts', function (utils) {
        ...
        utils.cssRule('chartsContainerHeight', '.edui-chart-container { height:'
            + (this.chartContainerHeight || 300) + 'px}');
        ...
        function render() {
            ...
            for (var i = 0, len = sources.length; i < len; i++) {
                ...
                renderChart(container, typeConfig[config.meta.chartType],
                    chartConfig);
    ...
    UE.parse.register('list', function (utils) {
        ...
        if (ols.length || uls.length) {
            ...
            customCss.push(selector + ' .list-paddingleft-2{padding-left:'
                + this.listDefaultPaddingLeft + 'px}');
            customCss.push(selector + ' .list-paddingleft-3{padding-left:'
                + this.listDefaultPaddingLeft * 2 + 'px}');

            utils.cssRule('list', selector + ' ol,' + selector
                + ' ul{margin:0;padding:0;}li{clear:both;}' + customCss.join(
                    '\n'), document);
        }
            ...
            utils.each(nodes, function (list) {
                if (list.className && /custom_/i.test(list.className)) {
                    ...
                    if (listStyle == 'dash' || listStyle == 'dot') {
                        utils.pushItem(customCss, selector + ' li.list-'
                            + customStyle[listStyle] + '{background-image:url('
                            + T.liiconpath + customStyle[listStyle] + '.gif)}');
                        utils.pushItem(customCss, selector + ' ul.custom_'
                            + listStyle + '{list-style:none;} ' + selector
                            + ' ul.custom_' + listStyle
                        ...
                        utils.each(list.childNodes, function (li) {
                            if (li.tagName == 'LI') {
                                utils.pushItem(customCss, selector + ' li.list-'
                                    + customStyle[listStyle] + index
                                    + '{background-image:url(' + T.liiconpath
                                    + 'list-' + customStyle[listStyle] + index
                                    + '.gif)}');
                                index++;
                            }
                        });
                        utils.pushItem(customCss, selector + ' ol.custom_'
                            + listStyle + '{list-style:none;}' + selector
                            + ' ol.custom_' + listStyle
                            + ' li{background-position:0 3px;background-repeat:no-repeat}');
                    }
                    switch (listStyle) {
                        case 'cn':
