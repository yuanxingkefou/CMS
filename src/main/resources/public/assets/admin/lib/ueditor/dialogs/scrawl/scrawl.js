...
(function () {
    ...
    scrawl.prototype = {
        ...
        _addBoardListener: function (saveNum) {
            ...
            margin = parseInt(
                domUtils.getComputedStyle($G("J_wrap"), "margin-left"));
            drawStep.push(context.getImageData(0, 0, context.canvas.width,
                ...
                function (e) {
                    ...
                    switch (e.type) {
                        ...
                        case 'mouseup':
                            ...
                            if (!isMouseMove) {
                                context.arc(startX, startY, context.lineWidth,
                                    0, Math.PI * 2, false);
        ...
        _addOPerateListener: function (saveNum) {
            ...
            domUtils.on($G("J_nextStep"), "click", function () {
                if (drawStepIndex > 0 && drawStepIndex < drawStep.length) {
                    ...
                    drawStepIndex == drawStep.length && me.btn2disable(
                        "J_nextStep");
                }
            });
            ...
            domUtils.on($G("J_sacleBoard"), "click", function () {
                ...
                if (img) {
                    if (!scaleCon) {
                        ...
                        img.style.cssText = "position: absolute;top:"
                            + (canvas.height - img.height) / 2 + "px;left:"
                            + (canvas.width - img.width) / 2 + "px;";
        ...
        _clearSelection: function () {
            var list = ["J_operateBar", "J_colorBar", "J_brushBar",
                "J_eraserBar", "J_picBoard"];
...
(function () {
    ...
    ScaleBoy.prototype = {
        ...
        startScale: function (objElement) {
            ...
            Idom.style.cssText = 'visibility:visible;top:'
                + objElement.style.top + ';left:' + objElement.style.left
                + ';width:' + objElement.offsetWidth + 'px;height:'
        ...
        _validScaledProp: function (prop, value) {
            ...
            switch (prop) {
                case 'left':
                    return value < 0 ? 0 : (value + ele.clientWidth)
                    > wrap.clientWidth ? wrap.clientWidth - ele.clientWidth
                        : value;
                case 'top':
                    return value < 0 ? 0 : (value + ele.clientHeight)
                    > wrap.clientHeight ? wrap.clientHeight - ele.clientHeight
                        : value;
                case 'width':
                    return value <= 0 ? 1 : (value + ele.offsetLeft)
                    > wrap.clientWidth ? wrap.clientWidth - ele.offsetLeft
                        : value;
                case 'height':
                    return value <= 0 ? 1 : (value + ele.offsetTop)
                    > wrap.clientHeight ? wrap.clientHeight - ele.offsetTop
                        : value;
            }
        }
    };
...
function ue_callback(url, state) {
    ...
    function scale(img, max, oWidth, oHeight) {
        var width = 0, height = 0, percent, ow = img.width || oWidth,
...
function exec(scrawlObj) {
    if (scrawlObj.isScrawl) {
        ...
        if (!!base64) {
                ...
                onsuccess: function (xhr) {
                    if (!scrawlObj.isCancelScrawl) {
                        ...
                        if (responseObj.state == "SUCCESS") {
                            var imgObj = {},
                                url = editor.options.scrawlUrlPrefix
                                    + responseObj.url;
                            imgObj.src = url;
                            imgObj._src = url;
            ...
            var actionUrl = editor.getActionUrl(
                editor.getOpt('scrawlActionName')),
        ...
        addMaskLayer(lang.noScarwl
            ...
            + lang.continueBtn + "'  onclick='removeMaskLayer()'/>");
    }
}
