...
(function (Highcharts) {
    ...
    wrap(Axis.prototype, 'setAxisTranslation', function (proceed) {
        ...
        if (chart.options.chart.type === 'map' && !isXAxis && xAxis.transA
            !== UNDEFINED) {
    ...
    wrap(Chart.prototype, 'render', function (proceed) {
        ...
        if (mapNavigation.zoomOnMouseWheel) {
            Highcharts.addEvent(chart.container,
                document.onmousewheel === undefined ? 'DOMMouseScroll'
            ...
            if (options.enableButtons) {
                for (n in buttons) {
                    if (buttons.hasOwnProperty(n)) {
                        buttonOptions = merge(options.buttonOptions,
                            buttons[n]);

                        button = chart.renderer.button(buttonOptions.text, 0, 0,
                            outerHandler)
                        .attr({
    ...
    var MapAreaPoint = Highcharts.extendClass(Point, {
        ...
        applyOptions: function (options, x) {
            ...
            if (point.path && typeof point.path === 'string') {
                point.path = point.options.path = Highcharts.splitPath(
                    point.path);
            }
        ...
        onMouseOut: function () {
            ...
            if (duration && normalColor.rgba.length === 4
                ...
                point.colorInterval = setInterval(function () {
                    ...
                    if (graphic) {
                        graphic.attr('fill',
                            tweenColors(hoverColor, normalColor, pos));
                    }
                    if (pos >= 1) {
                        clearTimeout(point.colorInterval);
                    }
    ...
    seriesTypes.map = Highcharts.extendClass(seriesTypes.scatter, {
        ...
        drawLegendSymbolGradient: function (legend, item) {
            var spacing = legend.options.symbolPadding,
                ...
                gradientSize = this.chart.renderer.fontMetrics(
                    legend.options.itemStyle.fontSize).h,
            ...
            item.legendSymbol = this.chart.renderer.rect(
                horizontal ? box1.x + box1.width + spacing : box1.x
                    - gradientSize - spacing,		// Upper left x.
                box1.y,																				// Upper left y.
            ...
            if (horizontal) {
                legend.offsetWidth = box1.width + box2.width + box3.width
                    + (spacing * 2) + padding;
                legend.itemY = gradientSize + padding;
            } else {
                legend.offsetWidth = Math.max(box1.width, box3.width)
                    + (spacing) + box2.width + padding;
            ...
            while (i--) {
                if (typeof path[i] === 'number') {
                    if (even) { // even = x
                        ...
                        path[i] = Math.round(yAxis.len - yAxis.translate(
                            path[i]));
                    }
                    even = !even;
                }
            ...
            each(this.data, function (point) {
                ...
                if (valueRanges) {
                    ...
                    while (i--) {
                        ...
                        from = range.from;
                        to = range.to;
                        if ((from === UNDEFINED || value >= from) && (to
                                === UNDEFINED || value <= to)) {
                            color = range.color;
                            break;
                        }
                    ...
                    color = value === null ? seriesOptions.nullColor
        ...
        drawPoints: function () {
            ...
            each(series.data, function (point) {
                ...
                point.plotX = Math.round(minX + (maxX - minX) * pick(dataLabels
                    && dataLabels.anchorX, 0.5));
                point.plotY = Math.round(minY + (maxY - minY) * pick(dataLabels
                    && dataLabels.anchorY, 0.5));
        ...
        animateDrilldown: function (init) {
            var toBox = this.chart.plotBox,
                level = this.chart.drilldownLevels[this.chart.drilldownLevels.length
                - 1],
                fromBox = level.bBox,
