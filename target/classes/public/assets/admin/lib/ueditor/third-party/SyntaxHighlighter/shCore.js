...
(function (undefined) {
    ...
    XRegExp = function (pattern, flags) {
        ...
        while (pos < pattern.length) {
            ...
            if (tokenResult) {
                ...
                if (match = nativ.exec.call(nativeTokens[currScope],
                        pattern.slice(pos))) {
                    output.push(match[0]);
                    pos += match[0].length;
                } else {
        ...
        regex = RegExp(output.join(""),
            nativ.replace.call(flags, flagClip, ""));
        regex._xregexp = {
            source: pattern,
    ...
    XRegExp.cache = function (pattern, flags) {
        ...
        return XRegExp.cache[key] || (XRegExp.cache[key] = XRegExp(pattern,
            flags));
    };
    ...
    XRegExp.matchChain = function (str, chain) {
        return function recurseChain(values, level) {
            ...
            return ((level === chain.length - 1) || !matches.length) ? matches
    ...
    RegExp.prototype.test = function (str) {
        ...
        if (match && !compliantLastIndexIncrement && this.global
            && !match[0].length && (this.lastIndex > match.index)) {
            this.lastIndex--;
        }
        if (!this.global) {
            this.lastIndex = origLastIndex;
    ...
    String.prototype.replace = function (search, replacement) {
        ...
        if (Object.prototype.toString.call(replacement)
            === "[object Function]") {
            result = nativ.replace.call(this + "", search, function () {
                ...
                if (isRegex && search.global) {
                    search.lastIndex = arguments[arguments.length - 2]
                        + arguments[0].length;
                }
                        ...
                        if ($1) {
                            switch ($1) {
                                ...
                                case "&":
                                    return args[0];
                                case "`":
                                    return args[args.length - 1].slice(0,
                                        args[args.length - 2]);
                                case "'":
                                    return args[args.length
                                    - 1].slice(args[args.length - 2]
                                        + args[0].length);
                                ...
                                default:
                                    ...
                                    while ($1 > args.length - 3) {
                                        literalNumbers = String.prototype.slice.call(
                                            $1, -1) + literalNumbers;
    ...
    function runTokens(pattern, index, scope, context) {
        ...
        try {
            while (i--) { // Run in reverse order
                t = tokens[i];
                if ((scope & t.scope) && (!t.trigger || t.trigger.call(
                        context))) {
...
// references to SyntaxHighlighter.whatever...
        var sh = {
            ...
            regexLib: {
                ...
                doubleQuotedString: /"([^\\"\n]|\\.)*"/g,
                singleQuotedString: /'([^\\'\n]|\\.)*'/g,
                multiLineDoubleQuotedString: new XRegExp('"([^\\\\"]|\\\\.)*"',
                    'gs'),
            ...
            findElements: function (globalParams, element) {
                var elements = element ? [element] : toArray(
                    document.getElementsByTagName(sh.config.tagName)),
                    conf = sh.config,
                    result = []
                ;
                ...
                for (var i = 0; i < elements.length; i++) {
                    var item = {
                        ...
                        params: merge(globalParams,
                            parseParams(elements[i].className))
                    };
        ...
        function storeHighlighter(highlighter) {
            sh.vars.highlighters[getHighlighterId(
        ...
        function findElement(target, search, reverse /* optional */) {
            ...
            var nodes = reverse != true ? target.childNodes
                : [target.parentNode],
                propertyToFind = {'#': 'id', '.': 'className'}[search.substr(0,
                    1)]
                    || 'nodeName',
                expectedValue,
                found
        ...
        function wrapLinesWithCode(str, css) {
            ...
            if (css != null) {
                str = eachLine(str, function (line) {
                    ...
                    return spaces + '<code class="' + css + '">' + line
                        + '</code>';
                });
            }
        ...
        function getMatches(code, regexInfo) {
            ...
            while ((match = regexInfo.regex.exec(code)) != null) {
                ...
                if (typeof(resultMatch) == 'string') {
                    resultMatch = [new sh.Match(resultMatch, match.index,
                        regexInfo.css)];
                }
        ...
        function quickCodeHandler(e) {
            var target = e.target,
                highlighterDiv = findParentElement(target,
                    '.syntaxhighlighter'),
        ...
        sh.HtmlScript = function (scriptBrushName) {
            ...
            for (var i = 0; i < methodsToExpose.length; i++)
                // make a closure so we don't lose the name after i changes {(function () {
...
;(function () {
    ...
    function Brush() {
        ...
        this.regexList = [
            {
                regex: new RegExp('^\\s*#[#\\s]*\\.(' + this.getKeywords(
                    commenthelp) + ').*$', 'gim'),
            ...
            {
                regex: new RegExp('(?:\\b' + verbs.replace(/ /g, '\\b|\\b')
                    + ')-[a-zA-Z_][a-zA-Z0-9_]*', 'gmi'),
                css: 'functions'
            ...
            {
                regex: new RegExp('\\s+-(?!' + this.getKeywords(operators)
                    + ')[a-zA-Z_][a-zA-Z0-9_]*', 'gmi'),
                css: 'color1'
            },						// parameters	  
        ];
...
;(function () {
    ...
    function Brush() {
        function process(match, regexInfo) {
            ...
            if (match.attributes != null) {
                ...
                while ((attributes = regex.exec(code)) != null) {
                    ...
                    result.push(new constructor(attributes.value, match.index
                        + attributes.index + attributes[0].indexOf(
            ...
            if (tag != null) {
                result.push(
                    new constructor(tag.name, match.index + tag[0].indexOf(
                        tag.name), 'keyword')
                );
            }
